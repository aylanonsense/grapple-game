...
define([
	...
	PhysObj.prototype.undoCollision = function(collision) {
		if(this._frozen && !other._frozen) {
			other.pos.x -= collision.embed.x;
			other.pos.y -= collision.embed.y;
		}
		else if(other._frozen && !this._frozen) {
			...
			if(this.mass !== other.mass) {
				percent = this.mass / (this.mass + other.mass);
			}
			this.pos.x += collision.embed.x * percent;
			this.pos.y += collision.embed.y * percent;
			other.pos.x -= collision.embed.x * (1 - percent);
			other.pos.y -= collision.embed.y * (1 - percent);
		}
	};
	...
	PhysObj.prototype.freeze = function() {
		this._frozen = true;
	};
	PhysObj.prototype.unfreeze = function() {
		this._frozen = false;
	};
